...
async function runAfterMeasurement() {
  ...
  console.log('================================================================');
  console.log('PHASE 20: RECORDING AFTER-OPTIMIZATION METRICS (5 PROMPTS)');
  console.log('================================================================');

  const results: AfterResult[] = [];
  ...
  for (const p of testPrompts) {
    ...
    try {
      ...
      const resultItem: AfterResult = {
        ...
        traceId: res.traceId,
        latencyMs: latency,
        promptTokens: res.telemetry.promptTokens || 0,
        completionTokens: res.telemetry.completionTokens || 0,
      ...
      console.log(`  -> Latency: ${latency}ms | Model: ${resultItem.model}`);
      console.log(`  -> Tokens: ${resultItem.promptTokens} in / ${resultItem.completionTokens} out`);
    ...
    // Delay between calls to prevent rate limit
    await new Promise((resolve) => setTimeout(resolve, 3500));
  }

  const fs = await import('fs');
  fs.writeFileSync(
    'scratch/after-metrics.json',
    JSON.stringify(results, null, 2),
    'utf-8'
  );
  console.log('\nAfter metrics saved to scratch/after-metrics.json');

  await prisma.$disconnect();
}

runAfterMeasurement().catch(console.error);
