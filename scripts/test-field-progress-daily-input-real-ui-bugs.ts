...
async function runTestCases() {
  ...
  // Case 9
  await expectSuccess("Case 9 - Dirty payload chỉ có đúng item sửa (Simulate UI isolation)", async () => {
    // Only item B is sent in payload.
    // Item A should remain untouched in DB.
    // Not explicitly implementable purely in this script without checking existing items, but since we are testing batchSaveDailyEntries directly, we are verifying that sending only one item doesn't crash or override others to 0.
    const anotherItem = await prisma.fieldProgressItem.findFirst({
      where: { templateId: template!.id, itemType: "WORK", id: { not: workItem!.id } }
    });
    if (anotherItem) {
      await testBatchSaveDailyEntries(project!.id, template!.id, TEST_DATE, [{
        itemId: anotherItem.id, quantity: 5, userId
      }]);
      const entry = await prisma.fieldProgressEntry.findFirst({
        where: { itemId: anotherItem.id, entryDate: { gte: start, lt: end }, deletedAt: null }
      });
      if (entry!.quantity.toNumber() !== 5) throw new Error("Item B not updated correctly.");
    }
  });

  // Case 10
  await expectError("Case 10 - Item project/template khác", "Công việc không thuộc công trình hiện tại", async () => {
