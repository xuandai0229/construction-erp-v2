  ...
  return (
    <>
      <div role="dialog" aria-modal="true" className="fixed inset-0 z-[90] flex items-start justify-center bg-slate-900/60 backdrop-blur-sm sm:p-4 animate-in fade-in duration-200 overflow-hidden">
        <div className="bg-[var(--surface-subtle)] w-full h-full sm:h-auto sm:max-h-full sm:rounded-[var(--radius-xl)] shadow-2xl flex flex-col relative w-[calc(100vw-16px)] md:w-[min(1180px,calc(100vw-48px))] max-w-6xl overflow-hidden animate-in zoom-in-95 duration-300">
          
          {/* Sticky Header */}
          <div className="bg-[var(--surface)] px-4 sm:px-6 py-4 border-b border-[var(--border)] flex items-center justify-between z-20 shrink-0 shadow-[var(--shadow-card)] relative">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-blue-600 rounded-[var(--radius-xl)] flex items-center justify-center text-white shadow-[var(--shadow-elevated)] shadow-blue-500/20">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[12px] text-[var(--muted-foreground)]">Điền thông tin báo cáo thi công tại công trường</span>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6">
            <div className="max-w-none md:max-w-[1100px] mx-auto space-y-6">
              ...
              {/* Tabs */}
              <div className="bg-[var(--surface)] p-1 rounded-[var(--radius-xl)] border border-[var(--border)] shadow-[var(--shadow-card)] inline-flex mb-2">
                <button
                  onClick={() => updateField('type', 'DAILY')}
                  className={`px-6 py-2.5 rounded-[var(--radius-lg)] text-[14px] font-bold transition-all ${form.type === 'DAILY' ? 'bg-blue-50 text-blue-700 shadow-[var(--shadow-card)]' : 'text-[var(--muted-foreground)] hover:bg-[var(--surface-subtle)]'}`}
                >
                  Báo cáo ngày
                </button>
                <button
                  onClick={() => updateField('type', 'WEEKLY')}
                  className={`px-6 py-2.5 rounded-[var(--radius-lg)] text-[14px] font-bold transition-all ${form.type === 'WEEKLY' ? 'bg-blue-50 text-blue-700 shadow-[var(--shadow-card)]' : 'text-[var(--muted-foreground)] hover:bg-[var(--surface-subtle)]'}`}
                >
                  Báo cáo tuần
                </button>
              </div>
              ...
              {form.type === 'DAILY' ? (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                  <div className="bg-[var(--surface)] p-3.5 rounded-[var(--radius-xl)] border border-[var(--border)] shadow-[var(--shadow-card)] flex items-center gap-3">
                    <div className="bg-blue-50 p-2.5 rounded-[var(--radius-lg)] text-blue-600"><ListTodo className="w-5 h-5" /></div>
                    <div><p className="text-[11px] font-bold text-[var(--muted-foreground)] opacity-70 uppercase">Công việc</p><p className="font-bold text-[var(--foreground)] text-[18px] leading-tight">{selectedCount}</p></div>
                  </div>
                  <div className="bg-[var(--surface)] p-3.5 rounded-[var(--radius-xl)] border border-[var(--border)] shadow-[var(--shadow-card)] flex items-center gap-3">
                    <div className="bg-emerald-50 p-2.5 rounded-[var(--radius-lg)] text-emerald-600"><CheckCircle2 className="w-5 h-5" /></div>
                    <div><p className="text-[11px] font-bold text-[var(--muted-foreground)] opacity-70 uppercase">Tổng khối lượng nhập</p><p className="font-bold text-[var(--foreground)] text-[18px] leading-tight">{formatNumberSafe(totalQtyToday)}</p></div>
                  </div>
                  <div className="bg-[var(--surface)] p-3.5 rounded-[var(--radius-xl)] border border-[var(--border)] shadow-[var(--shadow-card)] flex items-center gap-3">
                    <div className="bg-purple-50 p-2.5 rounded-[var(--radius-lg)] text-purple-600"><FileImage className="w-5 h-5" /></div>
                    <div><p className="text-[11px] font-bold text-[var(--muted-foreground)] opacity-70 uppercase">Ảnh đính kèm</p><p className="font-bold text-[var(--foreground)] text-[18px] leading-tight">{form.photos.length}</p></div>
                  </div>
                  <div className="bg-[var(--surface)] p-3.5 rounded-[var(--radius-xl)] border border-[var(--border)] shadow-[var(--shadow-card)] flex items-center gap-3">
                    <div className="bg-amber-50 p-2.5 rounded-[var(--radius-lg)] text-amber-600"><Files className="w-5 h-5" /></div>
                    <div><p className="text-[11px] font-bold text-[var(--muted-foreground)] opacity-70 uppercase">Tài liệu</p><p className="font-bold text-[var(--foreground)] text-[18px] leading-tight">{form.attachments.length}</p></div>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                  <div className="bg-[var(--surface)] p-3.5 rounded-[var(--radius-xl)] border border-indigo-200 shadow-[var(--shadow-card)] flex items-center gap-3">
                    <div className="bg-indigo-50 p-2.5 rounded-[var(--radius-lg)] text-indigo-600"><CalendarRange className="w-5 h-5" /></div>
                    <div><p className="text-[11px] font-bold text-[var(--muted-foreground)] opacity-70 uppercase">Loại</p><p className="font-bold text-indigo-700 text-[14px] leading-tight">Báo cáo tuần</p></div>
                  </div>
                  <div className="bg-[var(--surface)] p-3.5 rounded-[var(--radius-xl)] border border-[var(--border)] shadow-[var(--shadow-card)] flex items-center gap-3">
              ...
              {form.type === 'DAILY' && (
                <ContentCard id="work-lines-section" className="overflow-hidden p-0 sm:p-0">
                  <div className="bg-[var(--surface-subtle)] px-5 py-4 border-b border-[var(--border)] flex flex-col sm:flex-row sm:items-center justify-between gap-3 sticky top-0 z-10">
                    <div className="flex items-center gap-2.5">
                      <div className="bg-blue-100 p-1.5 rounded-[var(--radius-lg)] text-blue-600">
                        <ListTodo className="w-5 h-5" />
                      </div>
                      <div>
                        <h3 className="font-bold text-[var(--foreground)] text-[15px]">Khối lượng thực hiện hôm nay</h3>
                        <p className="text-[12px] text-[var(--muted-foreground)] mt-0.5">Chọn công việc từ bảng khối lượng gốc của công trình</p>
                      </div>
                    </div>
                    ...
                    <Button
                      ...
                      className={`${!form.projectId ? 'bg-[var(--border)] text-[var(--muted-foreground)] opacity-70 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-[var(--shadow-card)]'} font-bold h-10 px-5 rounded-[var(--radius-xl)] transition-all whitespace-nowrap flex items-center`}
                  ...
                  <div className="p-4 sm:p-5 bg-[var(--surface-subtle)]">
                    {!form.projectId ? (
                      <div className="py-10 flex flex-col items-center justify-center text-center bg-[var(--surface)] rounded-[var(--radius-xl)] border border-dashed border-[var(--border)]">
                        <div className="w-14 h-14 bg-[var(--surface-subtle)] rounded-full flex items-center justify-center mb-3">
                          <Building2 className="w-6 h-6 text-[var(--muted-foreground)] opacity-70" />
                        </div>
                        <h4 className="font-bold text-[var(--foreground)] text-[15px]">Chưa chọn công trình</h4>
                        <p className="text-[13px] text-[var(--muted-foreground)] mt-1 max-w-[300px]">
                      ...
                      <div className="py-10 flex flex-col items-center justify-center">
                        ...
                        <p className="text-[13px] text-[var(--muted-foreground)] font-medium">Đang tải danh sách công việc gốc...</p>
                      </div>
                    ) : form.workLines.length === 0 ? (
                      <div className="py-10 flex flex-col items-center justify-center text-center bg-[var(--surface)] rounded-[var(--radius-xl)] border border-dashed border-blue-200">
                        <div className="w-14 h-14 bg-blue-50 rounded-full flex items-center justify-center mb-3">
                          <ListTodo className="w-6 h-6 text-blue-400" />
                        </div>
                        <h4 className="font-bold text-[var(--foreground)] text-[15px]">Chưa có khối lượng trong báo cáo</h4>
                        <p className="text-[13px] text-[var(--muted-foreground)] mt-1 mb-4 max-w-[350px]">
                          Bấm Thêm khối lượng để chọn công việc từ bảng khối lượng gốc.
                        </p>
                        <Button
                          type="button"
                          onClick={() => setIsPickerOpen(true)}
                          className="bg-blue-50 text-blue-700 hover:bg-blue-100 font-bold h-9 px-5 rounded-[var(--radius-lg)]"
                        >
                      ...
                      <div className="space-y-4">
                        {errors.workLines && (
                          <div className="p-3 bg-red-50 border border-red-200 rounded-[var(--radius-xl)] text-red-600 text-[13px] font-medium flex items-start gap-2">
                        ...
                        {/* Desktop Table */}
                        <div className="hidden md:block overflow-x-auto border border-[var(--border)] rounded-[var(--radius-xl)] bg-[var(--surface)] shadow-[var(--shadow-card)]">
                          <table className="w-full text-left border-collapse min-w-[900px]">
                            <thead className="bg-[var(--surface-subtle)] border-b border-[var(--border)] text-[12px] uppercase text-[var(--muted-foreground)] font-bold sticky top-0 z-10">
                              <tr>
                            ...
                            <tbody className="text-[13px] align-top">
                              {form.workLines.map((line, idx) => {
                                ...
                                const inputClass = "w-full h-9 px-2.5 text-[13px] text-[var(--foreground)] bg-[var(--surface)] border border-[var(--border)] rounded focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-[var(--muted-foreground)] opacity-70";
                                
                                return (
                                  <tr key={idx} className={`border-b border-[var(--border)] hover:bg-[var(--surface-subtle)] transition-colors ${isOver ? 'bg-red-50/30' : ''}`}>
                                    <td className="px-3 py-4 text-center font-medium text-[var(--muted-foreground)] opacity-70">{idx + 1}</td>
                                    <td className="px-4 py-4">
                                      {line.categoryName && <div className="text-[10px] font-bold text-[var(--muted-foreground)] opacity-70 uppercase tracking-wider mb-1 line-clamp-1">{line.categoryName}</div>}
                                      <div className="font-bold text-[var(--foreground)] line-clamp-2">
                                    ...
                                    <td className="px-2 py-4 text-center font-medium text-[var(--muted-foreground)]">{line.unit}</td>
                                    <td className="px-3 py-4 text-right">
                                      <div className="font-medium text-[var(--muted-foreground)]">{design}</div>
                                      <div className="text-[11px] text-emerald-600 font-medium">{before + sameDay}</div>
                                    </td>
                                    <td className="px-3 py-4 text-right font-black text-[var(--foreground)]">{remaining}</td>
          ...
          <div className="bg-[var(--surface)] border-t border-[var(--border)] p-4 sm:px-6 shrink-0 z-20 shadow-[0_-4px_15px_rgba(0,0,0,0.05)]">
            <div className="max-w-none md:max-w-[1100px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 w-full">
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <Button
                  ...
                  className="flex-1 sm:flex-none h-11 rounded-[var(--radius-xl)] text-[var(--muted-foreground)] font-bold hover:bg-[var(--surface-subtle)] border-[var(--border)]"
                >
                  Hủy
                </Button>
                <Button
      ...
      {showConfirmClose && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-[var(--surface)] rounded-[var(--radius-xl)] shadow-xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6">
              <h3 className="text-lg font-bold text-[var(--foreground)] mb-2">Bạn muốn xử lý báo cáo đang nhập thế nào?</h3>
              <p className="text-[14px] text-[var(--muted-foreground)] mb-6">Có dữ liệu bạn đã nhập nhưng chưa được lưu lại.</p>
              ...
              <div className="flex flex-col gap-3">
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="w-full border-red-200 bg-red-50 hover:bg-red-100 text-red-600 font-bold h-11 rounded-[var(--radius-xl)]"
                >
                ...
                <Button 
                  ...
                  variant="ghost" 
                  className="w-full text-[var(--muted-foreground)] hover:bg-[var(--border)] font-bold h-11 rounded-[var(--radius-xl)]"
                >
                  Tiếp tục chỉnh sửa
                </Button>
              </div>
