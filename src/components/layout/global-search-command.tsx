...
import { Search, Building2, Bell, FileText, ClipboardCheck, ArrowRight, Loader2, X } from "lucide-react";
...
export function GlobalSearchCommand({ globalContext }: GlobalSearchCommandProps) {
  ...
  const searchPortal = isOpen && typeof document !== 'undefined' ? createPortal(
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-4 sm:pt-20 px-3 sm:px-4">
      ...
      <div 
        ref={panelRef}
        className="relative z-[101] w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/10 animate-in fade-in zoom-in-95 duration-150"
      >
        <div className="flex items-center border-b border-slate-100 px-4 h-14 relative">
          ...
          <div className="relative flex-1 flex items-center h-full">
            <input 
              ...
              data-lpignore="true"
              ref={inputRef}
              className="flex h-full w-full bg-transparent py-3 pl-3 pr-10 text-slate-900 outline-none placeholder:text-slate-400 sm:text-base font-normal"
        ...
        <div className="max-h-[65vh] overflow-y-auto p-3 custom-scrollbar">
          {!query.trim() && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-1">
              <div>
                <div className="px-3 py-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tìm nhanh</div>
                <div className="flex flex-col gap-1 mt-1">
                  <Link href="/projects" onClick={closeSearch} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-blue-600 transition-colors">
                    <Building2 className="h-4 w-4 text-slate-400" /> Công trình đang chọn
                  </Link>
                  <Link href="/reports" onClick={closeSearch} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-blue-600 transition-colors">
                    <FileText className="h-4 w-4 text-slate-400" /> Báo cáo Chỉ huy trưởng
                  </Link>
                  <Link href="/approvals" onClick={closeSearch} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-blue-600 transition-colors">
                    <ClipboardCheck className="h-4 w-4 text-slate-400" /> Hồ sơ chờ xử lý
                  </Link>
                </div>
              </div>
              <div>
                <div className="px-3 py-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Cần chú ý gần đây</div>
                  ...
                  <div className="flex flex-col gap-1 mt-1">
                    {globalContext.notifications.slice(0, 3).map((n) => (
                      <Link
                        ...
                        href={n.href || '#'}
                        onClick={closeSearch}
                        className="flex items-start gap-3 rounded-xl px-3 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-100 hover:text-blue-600 transition-colors group"
                      >
          ...
          {results.projects.length > 0 && (
            <div className="mb-4">
              <div className="px-3 py-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Công trình</div>
              {results.projects.map((p) => (
                <button
                  key={p.id}
                  onClick={() => handleProjectSelect(p.id)}
                  className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm hover:bg-blue-50/80 hover:text-blue-700 transition-colors group"
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="font-medium truncate text-slate-900 group-hover:text-blue-700">{p.name}</div>
          ...
          {results.notifications.length > 0 && (
            <div className="mb-4">
              <div className="px-3 py-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Cần xử lý & Phê duyệt</div>
              {results.notifications.map((n) => (
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="font-medium truncate text-slate-900 group-hover:text-blue-700">{n.title}</div>
          ...
          {results.reports.length > 0 && (
            <div className="mb-4">
              <div className="px-3 py-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Báo cáo Chỉ huy trưởng</div>
              {results.reports.map((r) => (
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="font-medium truncate text-slate-900 group-hover:text-blue-700">{r.title}</div>
