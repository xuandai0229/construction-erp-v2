...
export function PurchaseRequestPlaceholder({ lowStockCount }: { lowStockCount: number }) {
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/[0.03]">
      ...
      {lowStockCount > 0 ? (
        ...
        <aside className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-slate-600">
          <div className="flex items-start gap-3">
            <ShieldAlert className="mt-0.5 h-5 w-5 text-slate-400" />
            <div>
              <div className="text-sm font-bold text-slate-700">Chưa có gợi ý đề xuất</div>
              <p className="mt-1 text-sm leading-6">
