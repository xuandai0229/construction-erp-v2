...
export function ExecutiveKpiGrid({ data, onOpenDrawer }: {
  ...
  const atRiskCount = data.projectOverview.filter((project) => project.health === "AT_RISK" || project.health === "DELAYED").length;
  ...
  const kpis = [
    { id: "total-projects", label: "Tổng công trình", value: totalCount, subtext: "Trong phạm vi được phép xem", icon: Building2, tone: "blue" as IconColorTone, drawerType: "PROJECT_STATUS" as DrawerType },
    { id: "active-projects", label: "Đang thi công", value: activeCount, subtext: totalCount > 0 ? `${Math.round((activeCount / totalCount) * 100)}% danh mục` : "Chưa có công trình", icon: Hammer, tone: "emerald" as IconColorTone, drawerType: "PROJECT_STATUS" as DrawerType },
    { id: "risk", label: "Rủi ro tiến độ", value: atRiskCount, subtext: "Có dữ liệu hợp lệ cần theo dõi", icon: TriangleAlert, tone: "rose" as IconColorTone, drawerType: "RISK" as DrawerType },
    { id: "entries-today", label: "Khối lượng hôm nay", value: entriesToday, subtext: "Bản ghi hiện trường", icon: Hammer, tone: "violet" as IconColorTone, drawerType: "VOLUME" as DrawerType },
    { id: "actions", label: "Việc vận hành", value: actionCount, subtext: "Issue, nhiệm vụ và vật tư", icon: ListChecks, tone: "orange" as IconColorTone, drawerType: "ACTIONS" as DrawerType },
    { id: "documents-reports", label: "Báo cáo / Tài liệu", value: documentsAndReports, subtext: data.period.label, icon: ClipboardCheck, tone: "sky" as IconColorTone, drawerType: undefined },
  ];
    ...
    <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6" data-dashboard-kpis>
      {kpis.map((kpi) => {
        const content = <>
          ...
          <div className="mt-3 flex min-w-0 items-center gap-1.5 pt-1">
        ...
        const className = "group flex min-h-[115px] min-w-0 flex-col justify-between rounded-2xl border border-slate-200/80 bg-white p-4.5 text-left shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md";
