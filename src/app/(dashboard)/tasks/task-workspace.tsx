...
export function TaskWorkspace({ projects, tasks, actorId, initialProjectId, initialMine, readOnly = false }: { projects: Project[]; tasks: Task[]; actorId: string; initialProjectId: string | null; initialMine: boolean; readOnly?: boolean }) {
  ...
  return <main className="mx-auto w-full min-w-0 max-w-7xl space-y-6 px-3 py-4 sm:p-6">
    <header className="flex min-w-0 flex-wrap items-center justify-between gap-3"><div className="min-w-0"><h1 className="break-words text-2xl font-semibold">Điều hành công việc</h1><p className="text-sm text-muted-foreground">Nhiệm vụ được lưu theo công trình và tải lại từ dữ liệu thật.</p></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={mine} onChange={(event) => { const value = event.target.checked; setMine(value); refresh(projectId, value); }} /> Nhiệm vụ của tôi</label></header>
    <section className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.8fr)]">
      <div className="min-w-0 space-y-4"><div className="min-w-0 rounded-lg border bg-card p-4"><label className="block text-sm font-medium">Công trình</label><select className="mt-1 w-full min-w-0 max-w-full rounded border p-2" value={projectId} onChange={(event) => { setProjectId(event.target.value); refresh(event.target.value, mine); }}><option value="">Chọn công trình</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.code} — {project.name}</option>)}</select></div>
      ...
      <aside className="min-w-0 space-y-4">{!readOnly && <form onSubmit={(event) => run(() => create(event))} className="rounded-lg border bg-card p-4"><h2 className="font-medium">Tạo nhiệm vụ</h2><input required name="title" minLength={3} className="mt-3 w-full rounded border p-2" placeholder="Tên nhiệm vụ" disabled={pending || !projectId} /><textarea name="description" className="mt-2 w-full rounded border p-2" placeholder="Mô tả" disabled={pending || !projectId} /><div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2"><select name="priority" className="min-w-0 rounded border p-2" defaultValue="NORMAL"><option value="NORMAL">Bình thường</option><option value="HIGH">Cao</option><option value="URGENT">Khẩn</option></select><input name="deadline" type="datetime-local" className="min-w-0 rounded border p-2" /></div><button className="mt-3 rounded bg-primary px-3 py-2 text-primary-foreground disabled:opacity-50" disabled={pending || !projectId}>{pending ? "Đang lưu…" : "Tạo nhiệm vụ"}</button></form>}
      {selected && <section className="rounded-lg border bg-card p-4"><div className="flex justify-between gap-4"><div><h2 className="font-medium">{selected.title}</h2><p className="text-sm text-muted-foreground">v{selected.version} · {selected.lifecycle} / {selected.review}</p></div><button type="button" onClick={() => setSelectedId(null)}>Đóng</button></div><p className="mt-3 text-sm">{selected.description}</p><p className="mt-3 text-sm">Tiến độ: <strong>{selected.progressPercent}%</strong> · Người thực hiện: {selected.primaryAssignee?.name ?? "Chưa giao"}</p>{readOnly && <p className="mt-3 rounded border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-900">Chế độ giám sát — Chỉ xem</p>}<div className="mt-4 flex flex-wrap gap-2">
        {!readOnly && selected.creatorId === actorId && !selected.primaryAssigneeId && <button disabled={pending} onClick={() => { const assigneeId = window.prompt("ID người thực hiện"); if (assigneeId) run(() => callAction(selected, "ASSIGN", { primaryAssigneeId: assigneeId })); }} className="rounded border px-2 py-1">Giao việc</button>}
        {!readOnly && selected.primaryAssigneeId === actorId && selected.acceptance === "PENDING" && <button disabled={pending} onClick={() => run(() => callAction(selected, "ACCEPT", {}))} className="rounded border px-2 py-1">Nhận việc</button>}
        {!readOnly && selected.primaryAssigneeId === actorId && selected.acceptance === "ACCEPTED" && selected.lifecycle === "ASSIGNED" && <button disabled={pending} onClick={() => run(() => callAction(selected, "START", {}))} className="rounded border px-2 py-1">Bắt đầu</button>}
        {!readOnly && selected.primaryAssigneeId === actorId && selected.lifecycle === "IN_PROGRESS" && <button disabled={pending} onClick={() => { const progress = window.prompt("Tiến độ 0-100", String(selected.progressPercent)); if (progress !== null) run(() => callAction(selected, "UPDATE_PROGRESS", { progressPercent: Number(progress) })); }} className="rounded border px-2 py-1">Cập nhật tiến độ</button>}
        {!readOnly && selected.primaryAssigneeId === actorId && selected.lifecycle === "IN_PROGRESS" && <button disabled={pending} onClick={() => { const summary = window.prompt("Tóm tắt kết quả"); if (summary) run(() => callAction(selected, "SUBMIT", { summary })); }} className="rounded border px-2 py-1">Gửi kết quả</button>}
        {!readOnly && selected.reviewerId === actorId && selected.lifecycle === "SUBMITTED" && selected.currentSubmissionId && <button disabled={pending} onClick={() => { const reason = window.prompt("Lý do cần sửa"); if (reason) run(() => callAction(selected, "REQUEST_CHANGES", { submissionId: selected.currentSubmissionId, reason })); }} className="rounded border px-2 py-1">Yêu cầu sửa</button>}
        {!readOnly && selected.approverId === actorId && selected.lifecycle === "SUBMITTED" && selected.currentSubmissionId && <button disabled={pending} onClick={() => run(() => callAction(selected, "APPROVE_RESULT", { submissionId: selected.currentSubmissionId }))} className="rounded border px-2 py-1">Duyệt kết quả</button>}
        {!readOnly && selected.approverId === actorId && selected.review === "RESULT_APPROVED" && <button disabled={pending} onClick={() => run(() => callAction(selected, "CONFIRM_COMPLETION", {}))} className="rounded border px-2 py-1">Xác nhận hoàn thành</button>}
