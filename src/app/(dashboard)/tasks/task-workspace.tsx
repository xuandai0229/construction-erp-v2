...
export function TaskWorkspace({ projects, tasks, actorId, initialProjectId, initialMine, readOnly = false }: { projects: Project[]; tasks: Task[]; actorId: string; initialProjectId: string | null; initialMine: boolean; readOnly?: boolean }) {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.8fr)]">
      ...
      <aside className="space-y-4">{!readOnly && <form onSubmit={(event) => run(() => create(event))} className="rounded-lg border bg-card p-4"><h2 className="font-medium">Tạo nhiệm vụ</h2><input required name="title" minLength={3} className="mt-3 w-full rounded border p-2" placeholder="Tên nhiệm vụ" disabled={pending || !projectId} /><textarea name="description" className="mt-2 w-full rounded border p-2" placeholder="Mô tả" disabled={pending || !projectId} /><div className="mt-2 grid grid-cols-2 gap-2"><select name="priority" className="rounded border p-2" defaultValue="NORMAL"><option value="NORMAL">Bình thường</option><option value="HIGH">Cao</option><option value="URGENT">Khẩn</option></select><input name="deadline" type="datetime-local" className="rounded border p-2" /></div><button className="mt-3 rounded bg-primary px-3 py-2 text-primary-foreground disabled:opacity-50" disabled={pending || !projectId}>{pending ? "Đang lưu…" : "Tạo nhiệm vụ"}</button></form>}
