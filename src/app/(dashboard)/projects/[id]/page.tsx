  ...
  return (
    <div className="app-page space-y-5 max-w-[1600px] mx-auto pb-10">
      ...
      <div className="sm:hidden space-y-6 px-1">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[12px] font-black text-slate-500 uppercase tracking-widest">{project.code}</span>
          ...
          <h1 className="text-[20px] font-black text-slate-900 leading-snug line-clamp-2">
            {project.name}
          </h1>

          <div className="mt-5 flex flex-col gap-3">
            <div className="flex justify-between items-center border-b border-dashed border-slate-200 pb-3">
              <span className="text-[13px] text-slate-500 font-medium">Chủ đầu tư</span>
              <span className="text-[13px] font-bold text-slate-900 truncate max-w-[65%] text-right">{project.investor || "—"}</span>
            </div>
            <div className="flex justify-between items-center border-b border-dashed border-slate-200 pb-3">
              <span className="text-[13px] text-slate-500 font-medium">Địa điểm</span>
              <span className="text-[13px] font-bold text-slate-900 truncate max-w-[65%] text-right">{project.location || "—"}</span>
            </div>
            <div className="flex justify-between items-center border-b border-dashed border-slate-200 pb-3">
              <span className="text-[13px] text-slate-500 font-medium">Khởi công</span>
              <span className="text-[13px] font-bold text-slate-900">{project.startDate ? formatDateVN(project.startDate) : "—"}</span>
            </div>
            <div className="flex justify-between items-center border-b border-dashed border-slate-200 pb-3">
              <span className="text-[13px] text-slate-500 font-medium">Hoàn thành</span>
              <span className="text-[13px] font-bold text-slate-900">{project.endDate ? formatDateVN(project.endDate) : "—"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 pt-2">
          <Link href={`/projects/${project.id}/field-progress`} className="flex items-center bg-white border border-slate-200/80 rounded-[16px] p-3.5 shadow-sm active:scale-[0.98] transition-transform">
            <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 mr-3">
              <ListTree className="w-5 h-5"/>
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-[15px] font-bold text-slate-900 truncate">Hạng mục & Công việc</h3>
              <p className="text-[12px] text-slate-500 truncate mt-0.5">{hasWbs ? "Quản lý hạng mục thi công" : "Chưa thiết lập WBS"}</p>
            </div>
            <ChevronRight className="w-5 h-5 text-slate-400 shrink-0"/>
          </Link>

          {hasWbs ? (
            <Link href={`/projects/${project.id}/field-progress/daily`} className="flex items-center bg-white border border-slate-200/80 rounded-[16px] p-3.5 shadow-sm active:scale-[0.98] transition-transform">
              <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0 mr-3">
                <ClipboardCheck className="w-5 h-5"/>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-[15px] font-bold text-slate-900 truncate">Khối lượng thực hiện</h3>
                <p className="text-[12px] text-slate-500 truncate mt-0.5">Cập nhật khối lượng theo ngày</p>
              </div>
              <ChevronRight className="w-5 h-5 text-slate-400 shrink-0"/>
            </Link>
          ) : (
             <div className="flex items-center bg-slate-50 border border-slate-200/50 rounded-[16px] p-3.5 opacity-60">
              <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600/50 flex items-center justify-center shrink-0 mr-3">
                <ClipboardCheck className="w-5 h-5"/>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-[15px] font-bold text-slate-700 truncate">Khối lượng thực hiện</h3>
                <p className="text-[12px] text-slate-500 truncate mt-0.5">Cần thiết lập WBS trước</p>
              </div>
            </div>
          )}

          {hasWbs ? (
            <Link href={`/projects/${project.id}/field-progress/summary`} className="flex items-center bg-white border border-slate-200/80 rounded-[16px] p-3.5 shadow-sm active:scale-[0.98] transition-transform">
              <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 mr-3">
                <BarChart2 className="w-5 h-5"/>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-[15px] font-bold text-slate-900 truncate">Tổng hợp khối lượng</h3>
                <p className="text-[12px] text-slate-500 truncate mt-0.5">Lũy kế và tiến độ</p>
              </div>
              <ChevronRight className="w-5 h-5 text-slate-400 shrink-0"/>
            </Link>
          ) : (
            <div className="flex items-center bg-slate-50 border border-slate-200/50 rounded-[16px] p-3.5 opacity-60">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-[15px] font-bold text-slate-700 truncate">Tổng hợp khối lượng</h3>
                <p className="text-[12px] text-slate-500 truncate mt-0.5">Cần thiết lập WBS trước</p>
              </div>
            </div>
          )}
        ...
        <div className="bg-white border border-slate-200/80 rounded-[16px] p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3 border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              <FolderOpen className="w-4 h-4 text-amber-500" />
              <h2 className="text-[15px] font-bold text-slate-900">Hồ sơ & Tài liệu</h2>
            </div>
          ...
          <div className="space-y-2">
            {project.documentFolders.slice(0, 3).map(folder => (
              <Link key={folder.id} href={`/documents/${project.id}?folder=${folder.id}`} className="flex items-center justify-between gap-3 group bg-slate-50 rounded-lg p-2.5 hover:bg-slate-100 active:bg-slate-200 transition-colors">
                <span className="text-[13px] font-medium text-slate-700 truncate">{formatFolderName(folder.name)}</span>
                <ChevronRight className="w-4 h-4 text-slate-400" />
              </Link>
            ))}
            {project.documentFolders.length === 0 && (
              <div className="text-[13px] text-slate-500 py-2 text-center">Chưa có thư mục nào</div>
            )}
          </div>
        </div>
      </div>
      ...
      {/* =========================================
          ...
          ========================================= */}
      <div className="hidden sm:block rounded-[20px] bg-white border border-slate-200/70 shadow-[0_14px_40px_rgba(15,23,42,0.04)] p-5 relative overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        ...
        <div className="relative z-10 flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <h1 className="text-[18px] sm:text-[26px] font-bold text-slate-900 tracking-tight line-clamp-2 leading-tight">{project.name}</h1>
            {getStatusBadge(project.status)}
          </div>
          <div className="mt-2 sm:mt-3 flex items-center">
            <span className="inline-flex items-center gap-1 sm:gap-1.5 bg-slate-50 text-slate-500 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-[6px] text-[11px] sm:text-[13px] font-medium border border-slate-200/50">
              Mã: {project.code}
            </span>
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-wrap items-center gap-1.5 sm:gap-2 shrink-0">
          <Link href="/projects" className="inline-flex items-center justify-center rounded-[10px] sm:rounded-[12px] bg-white border border-slate-200/80 px-3 sm:px-4 py-1.5 text-[12px] sm:text-[13px] font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50 hover:text-slate-900 focus:outline-none h-[32px] sm:h-[36px]">
            Quay lại
          </Link>
          {canManage && (
            <Link href={`/projects/${project.id}/edit`} className="inline-flex items-center justify-center rounded-[10px] sm:rounded-[12px] bg-blue-600 px-3 sm:px-4 py-1.5 text-[12px] sm:text-[13px] font-semibold text-white shadow-sm transition-colors hover:bg-blue-700 focus:outline-none h-[32px] sm:h-[36px]">
              Sửa
            </Link>
          )}
          {canManage && (
      ...
      <div className="hidden sm:block pt-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          ...
          <Link href={`/projects/${project.id}/field-progress`} className="group flex flex-col rounded-[16px] sm:rounded-[20px] bg-white border border-slate-200/70 p-3.5 sm:p-4 shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-md hover:border-blue-200">
            ...
            <h3 className="font-bold text-slate-900 text-[13px] sm:text-[14px] mb-1">Hạng mục & Công việc</h3>
            <p className="text-[12px] sm:text-[12.5px] text-slate-500 leading-relaxed mb-3 line-clamp-2 sm:line-clamp-none">Quản lý danh mục hạng mục, công việc, đơn vị và khối lượng thiết kế của công trình.</p>
            <div className="mt-auto pt-2.5 sm:pt-3 border-t border-slate-100 flex items-center justify-between">
              <span className={`text-[11px] sm:text-[12px] font-semibold ${hasWbs ? 'text-emerald-600' : 'text-slate-400'}`}>
                {hasWbs ? 'Đã thiết lập' : 'Chưa thiết lập'}
              </span>
              <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
          ...
          {hasWbs ? (
            <Link href={`/projects/${project.id}/field-progress/daily`} className="group flex flex-col rounded-[16px] sm:rounded-[20px] bg-white border border-slate-200/70 p-3.5 sm:p-4 shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-md hover:border-emerald-200">
              ...
              <h3 className="font-bold text-slate-900 text-[13px] sm:text-[14px] mb-1">Khối lượng thực hiện</h3>
              <p className="text-[12px] sm:text-[12.5px] text-slate-500 leading-relaxed mb-3 line-clamp-2 sm:line-clamp-none">Cập nhật và theo dõi khối lượng thi công thực tế theo từng ngày.</p>
              <div className="mt-auto pt-2.5 sm:pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[11px] sm:text-[12px] font-semibold text-slate-500">
                  Cập nhật khối lượng
                </span>
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors">
                  <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 group-hover:translate-x-0.5 transition-transform" />
                </div>
              </div>
            </Link>
          ) : (
            <div aria-disabled="true" className="group flex flex-col rounded-[16px] sm:rounded-[20px] bg-slate-50/70 border border-slate-200/50 p-3.5 sm:p-4 cursor-not-allowed opacity-60">
              ...
              <h3 className="font-bold text-slate-700 text-[13px] sm:text-[14px] mb-1">Khối lượng thực hiện</h3>
              <p className="text-[12px] sm:text-[12.5px] text-slate-500 leading-relaxed mb-3 line-clamp-2 sm:line-clamp-none">Cập nhật và theo dõi khối lượng thi công thực tế theo từng ngày.</p>
              <div className="mt-auto pt-2.5 sm:pt-3 border-t border-slate-200 flex items-center justify-between">
                <span className="text-[11px] sm:text-[12px] font-semibold text-slate-400">
                  Cần thiết lập WBS
                </span>
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-300">
          ...
          {hasWbs ? (
            <Link href={`/projects/${project.id}/field-progress/summary`} className="group flex flex-col rounded-[16px] sm:rounded-[20px] bg-white border border-slate-200/70 p-3.5 sm:p-4 shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-md hover:border-indigo-200">
              ...
              <h3 className="font-bold text-slate-900 text-[13px] sm:text-[14px] mb-1">Tổng hợp khối lượng</h3>
              <p className="text-[12px] sm:text-[12.5px] text-slate-500 leading-relaxed mb-3 line-clamp-2 sm:line-clamp-none">Theo dõi khối lượng thiết kế, khối lượng đã thực hiện, lũy kế và tỷ lệ hoàn thành.</p>
              <div className="mt-auto pt-2.5 sm:pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[11px] sm:text-[12px] font-semibold text-slate-500">
                  Báo cáo chi tiết
                </span>
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                  <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 group-hover:translate-x-0.5 transition-transform" />
                </div>
              </div>
            </Link>
            ...
            <div aria-disabled="true" className="group flex flex-col rounded-[16px] sm:rounded-[20px] bg-slate-50/70 border border-slate-200/50 p-3.5 sm:p-4 cursor-not-allowed opacity-60">
              ...
              <h3 className="font-bold text-slate-700 text-[13px] sm:text-[14px] mb-1">Tổng hợp khối lượng</h3>
              <p className="text-[12px] sm:text-[12.5px] text-slate-500 leading-relaxed mb-3 line-clamp-2 sm:line-clamp-none">Theo dõi khối lượng thiết kế, khối lượng đã thực hiện, lũy kế và tỷ lệ hoàn thành.</p>
              <div className="mt-auto pt-2.5 sm:pt-3 border-t border-slate-200 flex items-center justify-between">
                <span className="text-[11px] sm:text-[12px] font-semibold text-slate-400">
                  Cần thiết lập WBS
