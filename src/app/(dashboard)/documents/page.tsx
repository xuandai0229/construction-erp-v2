          ...
          <form className="flex max-w-2xl items-center gap-2 overflow-x-auto pb-1 sm:overflow-visible sm:pb-0" method="GET" action="/documents">
            <div className="relative min-w-[220px] flex-1">
              ...
              <input autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false} data-1p-ignore="true" data-lpignore="true" 
                ...
                className="form-control h-11 pl-10 pr-4 rounded-xl border-slate-200 text-slate-900 bg-white"
              />
            </div>
            <div className="flex shrink-0 items-center gap-2">
              <Button type="submit" variant="primary" className="h-11 px-6 font-bold">
                Lọc dữ liệu
              </Button>
              {q && (
                <Link href="/documents">
                  <Button variant="secondary" className="h-11 font-medium">
                    Xóa
                  </Button>
                </Link>
              )}
              ...
              return (
                <div className={`${
                  ...
                  {projects.map(project => {
                    ...
                    const executionUnit = typeof project.sourceMetadata === "object" && project.sourceMetadata && "unit" in project.sourceMetadata ? String((project.sourceMetadata as { unit?: unknown }).unit ?? "") : null;

                    return (
                      <Link href={`/documents/${project.id}`} key={project.id} className="block group outline-none">
                        <div className={`${
                          density === 'list'
                            ? 'flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 hover:border-blue-300 hover:shadow-md transition-all'
                            : density === 'compact'
                              ? 'rounded-xl border border-slate-200 bg-white p-4 hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer h-full flex flex-col'
                              : 'rounded-xl border border-slate-200 bg-white p-5 hover:border-blue-300 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer h-full flex flex-col'
                        }`}>
                          ...
                          {density === 'list' ? (
                            ...
                            <>
                              <div className="flex items-start gap-3.5 mb-4">
                                <div className={`${density === 'compact' ? 'h-10 w-10 rounded-lg' : 'h-12 w-12 rounded-xl'} bg-blue-50 flex items-center justify-center group-hover:bg-blue-100 group-hover:scale-105 transition-all duration-300 shrink-0 ring-4 ring-white shadow-xs`}>
                                  <FolderOpen className={`${density === 'compact' ? 'h-5 w-5' : 'h-6 w-6'} text-blue-600`} />
                                </div>
                                <ProjectIdentity 
                                  name={project.name}
                                  displayName={project.displayName}
