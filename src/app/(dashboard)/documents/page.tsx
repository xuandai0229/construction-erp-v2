  ...
  return (
    <div className="app-page space-y-6">
      ...
      <ContentCard className="overflow-hidden">
        ...
        {projects.length > 0 ? (
          <div className="p-5 sm:p-6 bg-[var(--surface-subtle)]">
            {(() => {
                ...
                <div className={`${
                  ...
                  {projects.map(project => (
                    <Link href={`/documents/${project.id}`} key={project.id} className="block group outline-none">
                      <div className={`${
                        density === 'list'
                          ? 'flex items-center justify-between rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] px-4 py-3 hover:border-blue-300 hover:shadow-[var(--shadow-card)] transition-all'
                          : density === 'compact'
                            ? 'rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] p-4 hover:border-blue-300 hover:shadow-[var(--shadow-elevated)] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer h-full flex flex-col'
                            : 'rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] p-5 hover:border-blue-300 hover:shadow-[var(--shadow-elevated)] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer h-full flex flex-col'
                      }`}>
                        ...
                        {density === 'list' ? (
                          <>
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="h-9 w-9 shrink-0 rounded-[var(--radius-lg)] bg-blue-50/80 flex items-center justify-center">
                                <FolderOpen className="h-4 w-4 text-blue-600" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <h3 className="font-bold text-[var(--foreground)] group-hover:text-blue-600 truncate text-[14px] leading-tight transition-colors">{project.name}</h3>
                                <p className="text-[12px] text-[var(--muted-foreground)] font-medium truncate mt-0.5">{project.code}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-6 shrink-0">
                              <div className="flex items-center gap-1.5 text-[12px] text-[var(--muted-foreground)] font-medium">
                                <FolderOpen className="h-3.5 w-3.5 text-amber-500" />
                                {project._count.documentFolders} thư mục
                              </div>
                              <div className="flex items-center gap-1.5 text-[12px] text-[var(--muted-foreground)] font-medium w-[100px]">
                          ...
                          <>
                            <div className="flex items-start gap-3.5 mb-4">
                              <div className={`${density === 'compact' ? 'h-10 w-10 rounded-[var(--radius-lg)]' : 'h-12 w-12 rounded-[var(--radius-xl)]'} bg-blue-50/80 flex items-center justify-center group-hover:bg-blue-100/80 group-hover:scale-105 transition-all duration-300 shrink-0 ring-4 ring-white shadow-[var(--shadow-card)]`}>
                                <FolderOpen className={`${density === 'compact' ? 'h-5 w-5' : 'h-6 w-6'} text-blue-600 drop-shadow-[var(--shadow-card)]`} />
                              </div>
                              <div className="flex-1 pt-1 min-w-0">
                                <h3 className="font-bold text-[var(--foreground)] group-hover:text-blue-600 truncate text-[14px] sm:text-[15px] leading-tight mb-1 transition-colors" title={project.name}>{project.name}</h3>
                                <p className="text-[12px] sm:text-[13px] text-[var(--muted-foreground)] font-medium flex items-center gap-1.5 truncate" title={project.code}>
                                  <Building2 className="h-3.5 w-3.5 text-[var(--muted-foreground)] opacity-70 shrink-0" />
                                  <span className="truncate">{project.code}</span>
                                </p>
                              </div>
                            </div>
                            <div className="flex flex-col gap-3 mt-auto pt-4 border-t border-[var(--border)]/80">
                              <div className="flex items-center gap-5">
                                <div className="flex items-center gap-1.5">
                                  <div className="h-6 w-6 rounded-[var(--radius-md)] bg-amber-50 flex items-center justify-center shrink-0">
                                    <FolderOpen className="h-3.5 w-3.5 text-amber-600" />
                                  </div>
                                  <span className="text-[13px] font-semibold text-[var(--foreground)]">{project._count.documentFolders}</span>
                                  {density === 'comfortable' && <span className="text-[13px] text-[var(--muted-foreground)]">thư mục</span>}
                                </div>
                                <div className="flex items-center gap-1.5">
                              ...
                              {project.updatedAt && density === 'comfortable' && (
                                <div className="flex items-center gap-1.5 text-[12px] font-medium text-[var(--muted-foreground)] opacity-70 mt-0.5">
