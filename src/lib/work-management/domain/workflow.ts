...
export type TransitionDecision = { allowed: boolean; nextState?: TaskState; errorCode?: WorkManagementErrorCode; requiredEvents: DomainEvent[]; requiredSideEffects: SideEffectIntent[]; requiredPermission?: string };
...
function allowed(input: TransitionInput, nextState: TaskState): TransitionDecision { return { allowed: true, nextState, requiredPermission: permissionByAction[input.action], requiredEvents: [{ type: eventByAction[input.action], taskId: input.taskId }], requiredSideEffects: ["WRITE_TASK_ACTIVITY", "WRITE_AUDIT_LOG", "SEND_NOTIFICATION"] }; }
...
export function evaluateTaskTransition(input: TransitionInput): TransitionDecision {
  ...
  switch (input.action) {
    ...
    case "CANCEL": return !["CANCELLED", "COMPLETED", "ARCHIVED"].includes(state.lifecycle) ? allowed(input, withState(state, { lifecycle: "CANCELLED" })) : denied(input, "TASK_INVALID_TRANSITION");
    case "ARCHIVE": return ["COMPLETED", "CANCELLED"].includes(state.lifecycle) ? allowed(input, withState(state, { lifecycle: "ARCHIVED" })) : denied(input, "TASK_INVALID_TRANSITION");
    case "RESTORE": return state.lifecycle === "ARCHIVED" ? allowed(input, withState(state, { lifecycle: "DRAFT", acceptance: "NOT_REQUIRED", execution: "ACTIVE", review: "NOT_SUBMITTED", handover: "NONE", waitingReason: null })) : denied(input, "TASK_INVALID_TRANSITION");
    case "REQUEST_HANDOVER": return state.handover === "NONE" ? allowed(input, withState(state, { handover: "PENDING_TO_USER" })) : denied(input, "TASK_HANDOVER_CONFLICT");
